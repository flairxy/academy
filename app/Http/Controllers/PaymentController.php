...
class PaymentController extends Controller
{
    ...
    public function createPayment(Request $request)
    {
        ...
        $transaction->setAmount($amount)->setItemList($itemList)
            ->setDescription('Billionaires code subscription');
        ...
        $category = Category::whereAmount($request->amount)->first();
    ...
    public function confirmPayment(Request $request)
    {
        ...
        $subscription = Subscription::whereUserId(Auth::user()->id)->latest()->first();
        $subscription->transaction_id = $request->query('paymentId');
        $subscription->save();
        ...
        $totalAmount = intval($transaction[0]->getAmount()->total);
        if ($result->getState() != 'approved') {
            return redirect('/checkout')->withError('Payment was not successful. Please try again');
        }

        $sub = Subscription::whereTransactionId($request->query('paymentId'))->first();
        $cat = Category::whereAmount($totalAmount)->first();
        $sub->status = 1;
        $sub->category_id = $cat->id;
        $sub->expires_at = Carbon::now()->addMonths($cat->duration);
        $sub->save();
        return redirect('/checkout')->withSuccess('Payment successful.');
    }

    public function index()
    {
        $categories = Category::all();
        return view('checkout-page', ['categories' => $categories]);
    }
}
